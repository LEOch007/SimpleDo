...
import com.gy.linjliang.simpledo.widget.OnWheelChangedListener;
import com.gy.linjliang.simpledo.widget.OnWheelScrollListener;
import com.gy.linjliang.simpledo.widget.WheelView;
import com.gy.linjliang.simpledo.widget.adapters.AbstractWheelTextAdapter;
...
//                mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
        ...
        switch (v.getId()) {
            case R.id.sure_btn://确定选择按钮监听
                if (mBlnTimePickerGone) {
                    dateChooseInterface.getDateTime(strTimeToDateFormat(mHourStr, mMinuteStr), mBlnBeLongTerm);
                } else {
    ...
    private String strTimeToDateFormat(String hourStr, String minuteStr) {

        return hourStr.substring(0,hourStr.length()-1) + ":" + minuteStr.substring(0,minuteStr.length()-1);
    }
